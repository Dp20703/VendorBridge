...
export const Comparison: React.FC = () => {
  ...
  const activeQuotes = quotations.filter(
    (q) => q.rfqId === selectedRfqId && q.status === "Submitted",
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold font-display tracking-tight text-white">
            Advanced Bid Evaluation Matrix
          </h2>
          <p className="text-xs text-slate-400 font-sans mt-0.5">
            Automatically dissect and index competitive responses
          </p>
        </div>

        {/* Selected RFP Tender selector */}
        <div className="flex items-center gap-2 select-none">
          <span className="text-xs text-slate-400 font-sans font-medium shrink-0">
            Compare RFP:
          </span>
          <select
            value={selectedRfqId}
      ...
      {!activeRfq ? (
        <div className="py-12 text-center rounded-xl bg-slate-950 p-6">
          No RFQ located in sandbox.
        </div>
        ...
        <div className="py-16 text-center border border-dashed dark:border-slate-800 rounded-xl max-w-2xl mx-auto p-4 font-sans select-none">
          ...
          <h3 className="text-sm font-bold text-slate-200">
            No Bids Pending Sourcing Benchmarks
          </h3>
          <p className="text-xs text-slate-400 mt-1">
            Standard bids assigned for "{activeRfq.title}" have already been
            processed, or are drafting. To demonstrate this, review the top
            dropdown and ensure **RFQ-001** is active.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 relative select-none">
            {activeQuotes.map((quote) => {
              ...
              const isFastestDelivery =
                quote.id === highlights.fastestDeliveryId;
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="my-5 p-3 rounded-lg dark:bg-slate-950/50 light:bg-slate-100 grid grid-cols-2 gap-4 font-mono select-none">
                      <div>
                        <span className="text-[10px] text-slate-500 dark:text-slate-400 font-sans block mb-0.5">
                          Bid pricing quote
                        </span>
                      ...
                      <div>
                        ...
                        <span className="text-base font-black text-slate-100">
                          {quote.deliveryTimeline} calendar days
                        </span>
                    ...
                    <p className="text-xs text-slate-400 font-sans font-medium leading-relaxed italic line-clamp-3">
                      "
                      {quote.notes ||
                        "No custom SLA remarks submitted with price table."}
                      "
                    </p>
                    ...
                    <div className="mt-5 pt-4 border-t dark:border-slate-850/60 light:border-slate-200 font-sans">
                      <span className="text-[10px] font-mono font-bold text-slate-500 uppercase block mb-2.5">
                        Specific Unit Price Index
                      </span>
                      <div className="space-y-2">
                        {quote.items.map((it, idx) => {
                          const originalItem = activeRfq.items.find(
                            (ri) => ri.id === it.rfqItemId,
                          );
                          return (
                            <div
                              key={idx}
                              className="flex justify-between text-xs font-sans dark:text-slate-400"
                            >
                    ...
                    {isProposed ? (
                      ...
                      <button
                        onClick={() => {
                          ...
                          alert(
                            `Quotation comparison dispatched! Pending Approval request files with Management Dashboard (Persona Sophia).`,
                          );
                        }}
          ...
          <div className="relative overflow-hidden p-6 rounded-3xl border-2 dark:border-emerald-500/30 dark:bg-slate-900/85 light:border-emerald-250 light:bg-emerald-50/70 shadow-2xl transition-all duration-300">
              ...
              <div className="flex items-start gap-3.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono font-semibold text-slate-505 dark:text-slate-400">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Utilizes deep multi-parameter neural models to score, filter
                    risk, verify SLA compliance, and suggest optimization
                    strategies.
                  </p>
                </div>
              </div>
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    if (!response.ok) {
                      throw new Error(
                        "Sourcing Evaluator returned an error response.",
                      );
                    }
                    ...
                    setAiError(
                      err.message || "Failed to finalize evaluation matrix.",
                    );
                    setIsAiLoading(false);
                  }
                }}
            ...
            {!isAiLoading && !showSuccessAnimation && aiError && (
              <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-sans mt-2 relative z-10 flex items-start gap-2.5">
                ...
                <div>
                  <strong>Evaluation Interrupted:</strong> {aiError}. Sourcing
                  metrics calculated in programmatic fallback mode can always be
                  accessed.
                </div>
              </div>
            )}
              ...
              !aiError && (
                <div className="bg-slate-900/40 dark:bg-slate-950/60 border border-dashed dark:border-slate-800/80 p-8 rounded-2xl text-xs leading-relaxed text-slate-400 font-sans italic text-center select-text relative z-10 flex flex-col items-center justify-center">
                  ...
                  <p className="text-[11px] text-slate-400 max-w-sm mt-1">
                    Sourcing analytical matrices reside on-standby. Press
                    "Generate AI Recommendation" above to execute live cognitive
                    grading.
                  </p>
                </div>
              )}
            ...
            {!isAiLoading && !showSuccessAnimation && aiRecommendation && (
              <div className="space-y-6 pt-5 border-t dark:border-slate-800 border-slate-200 relative z-10 animate-fade-in">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b dark:border-slate-850/60 border-slate-250">
                  <div>
                    ...
                    <h5 className="text-lg font-black font-display text-white mt-0.5">
                      {aiRecommendation.recommendedVendor ||
                        activeQuotes.find(
                          (q) => q.id === aiRecommendation.recommendedQuoteId,
                        )?.vendorName ||
                    ...
                    <div className="relative w-12 h-12 shrink-0">
                      <svg className="w-12 h-12 -rotate-90">
                        ...
                        <circle
                          ...
                          strokeDashoffset={
                            125.6 -
                            (125.6 * (aiRecommendation.confidenceScore || 92)) /
                              100
                          }
                        />
                      </svg>
                      <span className="absolute inset-0 flex items-center justify-center font-mono text-[11px] font-black text-slate-100">
                        {aiRecommendation.confidenceScore || 92}%
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="text-xs font-bold text-emerald-400">
                        {aiRecommendation.confidenceScore >= 85
                          ? "Extreme Precision"
                          : "High Precision"}
                      </span>
                    </div>
                  ...
                  <div className="p-4 rounded-2xl dark:bg-slate-950/70 border dark:border-slate-850 bg-slate-100/50 border-slate-200">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-black text-emerald-500 font-mono">
                        {aiRecommendation.costScore ||
                          aiRecommendation.costEfficiency ||
                          90}
                        /100
                      </span>
                    </div>
                    <div className="w-full bg-slate-850 h-1.5 rounded-full mt-2.5 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${aiRecommendation.costScore || aiRecommendation.costEfficiency || 90}%`,
                        }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl dark:bg-slate-950/70 border dark:border-slate-850 bg-slate-100/50 border-slate-200">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-black text-amber-500 font-mono">
                        {aiRecommendation.speedScore ||
                          aiRecommendation.deliveryScore ||
                          90}
                        /100
                      </span>
                    </div>
                    <div className="w-full bg-slate-850 h-1.5 rounded-full mt-2.5 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${aiRecommendation.speedScore || aiRecommendation.deliveryScore || 90}%`,
                        }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl dark:bg-slate-950/70 border dark:border-slate-850 bg-slate-100/50 border-slate-200">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-black text-teal-400 font-mono">
                        {aiRecommendation.reliabilityScore || 90}/100
                      </span>
                    </div>
                    <div className="w-full bg-slate-850 h-1.5 rounded-full mt-2.5 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${aiRecommendation.reliabilityScore || 90}%`,
                        }}
                      />
                    </div>
                  </div>
                ...
                <div className="p-4 rounded-2xl border dark:border-emerald-500/20 bg-emerald-500/[0.03] text-xs font-sans text-slate-200 flex items-start gap-3 relative overflow-hidden shadow-inner">
                  ...
                  <div>
                    <strong className="text-emerald-400 font-mono uppercase text-[11px] tracking-wider block mb-0.5">
                      Critical AI recommendation highlight:
                    </strong>
                    <p className="text-slate-300 italic">
                      "
                      {aiRecommendation.keyHighlight ||
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                  ...
                  <div className="lg:col-span-4 p-4 rounded-2xl dark:bg-slate-950 border dark:border-slate-850 flex flex-col justify-between space-y-3 font-sans text-xs">
                    <div>
                      <span className="text-[10px] font-mono text-slate-500 uppercase font-black tracking-wider block mb-3">
                        AI RISK ANALYSIS & PROJECTIONS
                      </span>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] uppercase font-mono font-black border ${
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">
                            Financial Saving SLA:
                          </span>
                          <span className="text-emerald-400 font-mono font-bold font-display">
                            Optimal Cost Basis
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">
                            Vendor Compliance Rank:
                          </span>
                    ...
                    <div className="pt-3 border-t dark:border-slate-850/60 text-[10px] leading-relaxed text-slate-500">
                      * Projections calculated using automated matrix analyses
                      of history from 2,500 audited enterprise suppliers.
                    </div>
                  </div>

                  {/* Main AI Reasoning Text */}
                  <div className="lg:col-span-8 flex flex-col">
                    <span className="text-[10px] font-mono text-slate-505 dark:text-slate-500 uppercase font-black tracking-wider block mb-2.5">
                      Sourcing Decision Matrix & Insights
                    </span>
                    <div className="flex-1 text-xs leading-relaxed text-slate-350 select-text p-4 rounded-2xl dark:bg-slate-950/95 bg-slate-100 dark:border-slate-850 border border-slate-200 max-h-56 overflow-y-auto custom-scrollbar font-sans space-y-3">
                      {(() => {
                        const reasoningLines = Array.isArray(
                        ...
                        return reasoningLines.map((line: string, i: number) => {
                          if (
                            ...
                            return (
                              <h5
                                key={i}
                                className="text-sm font-black font-display text-white mt-4 mb-2 first:mt-0 pb-1 border-b dark:border-slate-900"
                              >
                                {line.replace(/###|\*\*/g, "").trim()}
                              </h5>
                            );
                          ...
                          if (
                            ...
                            return (
                              <div
                                key={i}
                                className="flex gap-2 items-start text-slate-300 pl-1 mt-1.5"
                              >
                  ...
                  if (isAlreadyProposed) {
                    return (
                      <div className="w-full text-center py-3 bg-gradient-to-r from-emerald-500/15 to-teal-500/15 border border-emerald-500/30 rounded-2xl text-emerald-400 font-mono text-[11px] font-extrabold uppercase animate-pulse">
                        🎯 AI RECOMMENDED DESIGNATED BID ASSIGNED FOR EXECUTIVE
                        AUDIT
                      </div>
                    );
                  }
                  ...
                  return (
                    <button
                      onClick={() => {
                        ...
                        alert(
                          `Gemini recommended sourcing contract dispatched! Sent directly to Management Approvals queue.`,
                        );
                      }}
                    ...
                    >
                      ...
                      <span>
                        Accept AI Recommendation & Generate Proposal Workflow
                      </span>
                    </button>
                  );
                })()}
