...
export const Rfqs: React.FC = () => {
  ...
  const [vendorBidModalOpen, setVendorBidModalOpen] = useState<RFQ | null>(
    null,
  );
  ...
  const handleCreateRFQSubmit = (e: React.FormEvent) => {
    ...
    if (assigned.length === 0)
      errs.assigned = "Please assign at least one supplier vendor.";
  ...
  const handleVendorBidSubmit = (e: React.FormEvent) => {
    ...
    const activeVendorId =
      currentUser.role === UserRole.VENDOR
        ? vendors.find((v) => v.email === currentUser.email)?.id ||
          "vendor-apex"
        : "vendor-apex";
  ...
  const displayedRfqs = isVendor
    ? rfqs.filter(
        (r) =>
          r.status === "Open" && r.assignedVendors.includes(activeVendorId),
      )
    : rfqs;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 font-sans mt-0.5">
            Sourcing and competitive bidding operations desk
          </p>
        </div>
      ...
      {isVendor && (
        <div className="p-4 rounded-xl border border-sky-500/20 bg-sky-500/[0.03] text-xs leading-relaxed dark:text-slate-350 select-none">
          <span className="font-extrabold uppercase font-mono text-sky-400 block mb-1">
            📋 SUPPLIER CONTRACT PORTAL
          </span>
          Each open RFQ assigned to your organization below contains structured
          technical briefs. Input your optimal pricing breakdown worksheet and
          expected SOW delivery timeline. Revised pricing sheets replace prior
          entries.
        </div>
      )}
      ...
      {displayedRfqs.length === 0 ? (
        <div className="py-16 text-center border border-dashed dark:border-slate-800 rounded-2xl">
          ...
          <h3 className="text-sm font-bold text-slate-200">
            No Active RFQs Listed
          </h3>
          <p className="text-xs text-slate-400 font-sans mt-1">
            There are no open requests assigned to your corporate profile at
            this stage.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 select-none">
          {displayedRfqs.map((rfq) => {
            const assignedSuppliers = vendors.filter((v) =>
              rfq.assignedVendors.includes(v.id),
            );
              ...
              >
                <div>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[9px] font-mono border ${rfqStatusStyles[rfq.status]} font-bold uppercase`}
                    >
                      {rfq.status}
                    </span>
                  </div>

                  <h3 className="text-sm font-bold text-slate-200 dark:text-slate-100 font-display mt-2.5 truncate">
                    {rfq.title}
                  </h3>
                  ...
                  <div className="mt-4 flex items-center gap-1.5 text-[11px] text-slate-400 font-mono">
                    ...
                    <span>
                      Closes: {new Date(rfq.deadline).toLocaleDateString()}
                    </span>
                  </div>
                  ...
                  <div className="mt-4 pt-3.5 border-t dark:border-slate-850/60 light:border-slate-200">
                    ...
                    <div className="space-y-1.5">
                      {rfq.items.map((it, idx) => (
                        <div
                          key={idx}
                          className="flex justify-between items-center text-xs dark:text-slate-300"
                        >
                          <span className="font-medium truncate max-w-[200px]">
                            {it.productName}
                          </span>
                          <span className="text-[11px] font-mono text-slate-450 shrink-0">
                            {it.quantity} {it.unit}
                          </span>
                        </div>
                      ))}
                ...
                <div className="mt-6 pt-4 border-t dark:border-slate-850/60 light:border-slate-200 flex items-center justify-between text-[11px]">
                  <div>
                    {!isVendor ? (
                      ...
                      <span className="text-emerald-500 font-medium font-sans">
                  ...
                  {isVendor && rfq.status === "Open" && (
                    <button
                      onClick={() => {
                        ...
                        const existingBid = quotientsForRfq(
                          rfq.id,
                          activeVendorId,
                        );
                        if (existingBid) {
                    ...
                    >
                      ...
                      <span>
                        {quotientsForRfq(rfq.id, activeVendorId)
                          ? "Revise Pricing Plan"
                          : "Submit Bid Sheet"}
                      </span>
                    </button>
          ...
          <div className="w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl relative font-sans">
            ...
            <div className="p-5 border-b border-slate-800">
              <h3 className="text-base font-bold text-white">
                Initialize New Multi-Item RFP Proposal
              </h3>
              <p className="text-[11px] text-slate-400 mt-0.5">
                Define material requirements, assign vetted firms, and set rigid
                timelines.
              </p>
            </div>
            ...
            >
              ...
              <div className="pt-3.5 border-t dark:border-slate-800/60">
                ...
                <div className="space-y-3">
                  {items.map((item, idx) => (
                    <div
                      key={idx}
                      className="p-3 rounded bg-slate-950 border border-slate-850 flex flex-col gap-3 relative"
                    >
                      {items.length > 1 && (
                        <button
                          type="button"
                      ...
                      <div className="grid grid-cols-12 gap-3">
                        ...
                        <div className="col-span-4 sm:col-span-2">
                          ...
                          <input
                            ...
                            value={item.unit}
                            onChange={(e) =>
                              handleItemValueChange(idx, "unit", e.target.value)
                            }
                            placeholder="Tons"
                            className="w-full px-2 py-1.5 rounded border border-slate-800 bg-slate-900 text-slate-100 text-xs text-center"
                          />
              ...
              <div className="pt-3.5 border-t dark:border-slate-800/60 font-sans">
                <label className="block text-[10px] uppercase font-mono font-bold tracking-wider text-slate-400 mb-2">
                  Assign Vetted Supplires *
                </label>
                {errors.assigned && (
                  <span className="text-[10px] text-red-500 block mb-2">
                    {errors.assigned}
                  </span>
                )}
                  ...
                  {vendors
                    ...
                    .map((vendor) => (
                      ...
                      >
                        ...
                        />
                        <span>
                          {vendor.companyName} ({vendor.rating.toFixed(1)}★)
                        </span>
                      </label>
                    ))}
                </div>
      ...
      {vendorBidModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs select-none">
          <div className="w-full max-w-xl bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl relative font-sans">
            ...
            <div className="p-5 border-b border-slate-800">
              <h3 className="text-base font-bold text-white">
                Interactive Sourcing pricing proposal
              </h3>
              <p className="text-[11px] text-slate-400 mt-0.5">
                Submit unit prices, notes, and commitments for RFQ:{" "}
                <span className="text-sky-400 font-bold">
                  "{vendorBidModalOpen.title}"
                </span>
              </p>
            </div>

            <form
              onSubmit={handleVendorBidSubmit}
              className="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar"
            >
              {/* Material prices lists */}
              <div className="space-y-3">
                <span className="text-[10px] font-mono font-bold uppercase text-slate-500">
                  Materials bidding pricing sheet:
                </span>

                {vendorBidModalOpen.items.map((it) => (
                  <div
                    key={it.id}
                    className="p-3 bg-slate-950 border border-slate-850 rounded-lg flex items-center justify-between gap-4"
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-slate-400 mt-0.5">
                        Assigned Target Quantity: {it.quantity} {it.unit}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="relative w-24">
                        <span className="text-[11px] font-mono text-slate-450 absolute left-2 top-2">
                          ₹
                        </span>
                        <input
                          type="number"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={bidTimeline}
                    onChange={(e) =>
                      setBidTimeline(parseInt(e.target.value) || 10)
                    }
              ...
              <div>
                <label className="block text-[10px] uppercase font-mono font-bold text-slate-400 mb-1.5 font-sans">
                  Corporate proposal notes / SOW conditions
                </label>
                <textarea
                  rows={2}
                  value={bidNotes}
              ...
              {(() => {
                ...
                return (
                  <div className="p-3.5 rounded-lg bg-slate-950 border border-slate-850/80 font-mono text-[11px] space-y-1.5 text-slate-400">
                    <span className="font-bold text-[9px] uppercase text-emerald-400 block mb-2 font-display tracking-wider">
                      Estimated ERP Bid Summary (18% standard GST)
                    </span>
